...
multimodel_data <- args[3];
Ls <- args[4];
# Ls <- Ls-1;
site <- args[5];
sitename <- gsub("_"," ",args[6]);
...
models <- c("cfsv2","echam4p5","multimodel")
models_names <- c("CFSv2","ECHAM4p5","Multimodel")
models_col <- c("blue","green","red")
models_pch <- c(1,20,4)
models_lty <- c(3,5,1)
...
multimodel=read.csv(multimodel_data, row.names=NULL);
...
for(l in 0:Ls){
  ...
  for(m in seq_along(models)){
    ...
    lines(model$M[model$L==l],model$CORR_CV_EN[model$L==l], type="b",pch=models_pch[m],lty=models_lty[m],col=models_col[m]);
    lines(model$M[model$L==l],1.96/sqrt(model$N0[model$L==l]-3) ,lty=1,col="gray77");
    polygon(
      c(0,0,model$M[model$L==l],13,13),
      c(0,1.96/sqrt(model$N0[model$L==l][1]-3),
      1.96/sqrt(model$N0[model$L==l]-3),
      1.96/sqrt(model$N0[model$L==l][12]-3),
      0),
      col = "#08040411");
...
for(l in 0:Ls){
  ...
  for(m in seq_along(models)){
    model <- get(models[m]);
    lines(model$M[model$L==l],model$MSSS_CV_EN[model$L==l], type="b",pch=models_pch[m],lty=models_lty[m],col=models_col[m]);
  }
